...
const Properties = () => { 
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      setLoading(true);
      try {
        // Only fetch from API for pagination and sorting
        const response = await getProperties(pagination, sort, {});
        ...
        if (filters.priceMin) {
          filtered = filtered.filter(property => 
            (property.price - property.discountPrice) >= filters.priceMin
          );
        }
        
        if (filters.priceMax) {
          filtered = filtered.filter(property => 
            (property.price - property.discountPrice) <= filters.priceMax
          );
        }
  ...
  }, [filters, searchTerm, allProperties, loading]);
