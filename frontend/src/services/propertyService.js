...
export const generateReport = async () => {
  try {
    ...
    properties.forEach(property => {
      ...
      row.innerHTML = `
        <td style="padding: 12px; border: 1px solid #ddd;">
          ...
          <div style="color: #666; font-size: 0.9em;">${property.beds || 0} beds, ${property.baths || 0} baths</div>
        </td>
    ...
    const averagePrice = properties.reduce((acc, p) => acc + (p.price || 0), 0) / totalProperties;
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">
          ...
          <div>
            <p style="margin: 5px 0;"><strong>Average Price:</strong> ${new Intl.NumberFormat('en-LK', {
              ...
              currency: 'LKR',
              maximumFractionDigits: 0
            }).format(isNaN(averagePrice) ? 0 : averagePrice)}</p>
          </div>
        </div>
      </div>
    `;
